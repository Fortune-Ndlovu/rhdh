...
OSD-GCP CI clusters do not reliably reach **ghcr.io**, so `install-dynamic-plugins` would fail when resolving `oci://ghcr.io/...` overlays. For `*osd-gcp*` jobs, merged Helm values are post-processed (see `config::strip_ghcr_dynamic_plugins_for_osd_gcp` in `.ci/pipelines/lib/config.sh`): **`ghcr.io` rows are removed**, **`includes` are cleared**, and every **`{{inherit}}` plugin row is removed** (with catalog disabled, there is no included base to inherit from). **`config::strip_orchestrator_plugin_entries_for_osd_gcp`** disables any remaining orchestrator-related entries.

**Operator OSD-GCP Backstage CR** (`rhdh-start-*-osd-gcp.yaml`) sets **`CATALOG_INDEX_IMAGE` to empty** on **`install-dynamic-plugins`**. The init container still mounts **`backstage-dynamic-plugins-*`** (operator defaults), which can include `oci` `{{inherit}}` rows. After the Backstage CR is applied, CI runs **`config::merge_osd_gcp_operator_dynamic_plugins`**: it strips ghcr/`{{inherit}}` from the operator ConfigMap copy, merges in the pipeline **`dynamic-plugins`** ConfigMap (custom wins per `package`), patches the operator ConfigMap, and restarts the Backstage Deployment. Playwright skips specs that require unavailable plugins when `JOB_NAME` contains `osd-gcp` (see `e2e-tests/playwright.config.ts`).
