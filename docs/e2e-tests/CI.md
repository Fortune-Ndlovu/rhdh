...
**Operator OSD-GCP Backstage CR** (`rhdh-start-*-osd-gcp.yaml`) sets **`CATALOG_INDEX_IMAGE` to empty** on **`install-dynamic-plugins`** only, so the catalog index image is not extracted or merged in that job. That avoids catalog-only `{{inherit}}` failing before user plugins are merged. Playwright skips specs that require unavailable plugins when `JOB_NAME` contains `osd-gcp` (see `e2e-tests/playwright.config.ts`).
