...
**Why the catalog index is not merged on OSD-GCP (operator):** We keep `includes: []` after stripping (no `dynamic-plugins.default.yaml` in includes). The installer only merges the catalog when that string is in `includes`; with an empty list the catalog is never merged, so we avoid (1) orchestrator `{{inherit}}` errors and (2) ghcr.io plugins from the catalog—OSD-GCP cannot reach ghcr.io (skopeo timeout). The stripped ConfigMap therefore supplies only the main plugins list (no ghcr.io, no orchestrator); the catalog may still be extracted by the image when `CATALOG_INDEX_IMAGE` is set by the operator, but it is not used.
