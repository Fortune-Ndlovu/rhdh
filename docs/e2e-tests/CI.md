...
OSD-GCP CI clusters do not reliably reach **ghcr.io**, so `install-dynamic-plugins` would fail when resolving `oci://ghcr.io/...` overlays. For `*osd-gcp*` jobs, merged Helm values are post-processed (see `config::strip_ghcr_dynamic_plugins_for_osd_gcp` in `.ci/pipelines/lib/config.sh`): every plugin whose `package` references `ghcr.io` or uses `{{inherit}}` is removed, and orchestrator-related plugin rows are stripped. Playwright skips specs that require those plugins when `JOB_NAME` contains `osd-gcp` (see `e2e-tests/playwright.config.ts`).

**Why the catalog index is still used on OSD-GCP (operator):** On regular OCP, `includes` contains `dynamic-plugins.default.yaml`, so the installer replaces it with the extracted catalog and merges all catalog plugins; the catalog provides explicit versions, so `{{inherit}}` in the main config finds a base. On OSD-GCP we clear orchestrator from the main plugins list, but the catalog (when loaded) still references the orchestrator with `{{inherit}}`, which would fail if there is no base. To keep using the catalog (so all non-orchestrator plugins from the index are available) without editing the installer script, we set `includes` to `["orchestrator-base.yaml", "dynamic-plugins.default.yaml"]` and add an `orchestrator-base.yaml` entry to the dynamic-plugins ConfigMap that defines the orchestrator plugin with an explicit version (`1.10`) and `disabled: true`. The installer processes includes in order, so the catalog’s `{{inherit}}` entry then finds this base. The RHDH operator must mount the full ConfigMap (all keys) so that `orchestrator-base.yaml` is present next to `dynamic-plugins.yaml` in the init container.
